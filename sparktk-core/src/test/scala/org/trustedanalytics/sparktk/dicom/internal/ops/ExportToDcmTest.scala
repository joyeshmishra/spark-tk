...
class ExportToDcmTest extends TestingSparkContextWordSpec with Matchers {
  ...
  "ExportToDcm" should {
    "create new .dcm from metadata and pixeldata and export to specified hdfs/local path" in {
      val dicom = importDcm(sparkContext, "../spark-tk/integration-tests/datasets/dicom_uncompressed")

      val destPath = "../spark-tk/integration-tests/tests/sandbox/dicom_export"
